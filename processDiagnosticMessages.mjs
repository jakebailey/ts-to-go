...
const outputFilePath = "output/diagnostics/diagnostics_generated.go";
...
function buildInfoFileOutput(messageTable) {
    const result = [
        `// GENERATED BY processDiagnosticMessages.mjs; DO NOT EDIT`,
        `package diagnostics`,
        ``,
        `type Category int32`,
        "",
    ...
    messageTable.forEach(
        ({ code, category, reportsUnnecessary, elidedInCompatabilityPyramid, reportsDeprecated }, name) => {
            ...
            result.push(
                `var ${propName} = &Message{code: ${code}, category: Category${category}, key: "${key}", text: ${
                    JSON.stringify(name)
