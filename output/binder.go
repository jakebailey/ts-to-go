...
func (b *Binder) createSymbol(flags SymbolFlags, name __String) Symbol {
	b.symbolCount++
		...
		if !symbol {
			...
			if symbol.isReplaceableByMethod {
				...
				forEach(symbol.declarations, func(declaration Declaration, index number) {
					...
					b.file.bindDiagnostics.push(__COND__(multipleDefaultExports, addRelatedInfo(diag, b.createDiagnosticForNode(declarationName, __COND__(index == 0, Diagnostics.Another_export_default_is_here, Diagnostics.and_here))), diag))
...
func (b *Binder) declareModuleMember(node Declaration, symbolFlags SymbolFlags, symbolExcludes SymbolFlags) Symbol {
	hasExportModifier := !!(getCombinedModifierFlags(node) & ModifierFlagsExport) || b.jsdocTreatAsExported(node)
	if symbolFlags & SymbolFlagsAlias {
		if node.kind == SyntaxKindExportSpecifier || (node.kind == SyntaxKindImportEqualsDeclaration && hasExportModifier) {
			...
			return b.declareSymbol(b.container.locals /*parent*/, nil, node, symbolFlags, symbolExcludes)
		}
	} else {
		...
		// We shouldn't add symbols for JSDoc nodes if not in a JS file.
		if !isAmbientModule(node) && (hasExportModifier || b.container.flags&NodeFlagsExportContext) {
			if !canHaveLocals(b.container) || !b.container.locals || (hasSyntacticModifier(node, ModifierFlagsDefault) && !b.getDeclarationName(node)) {
				return b.declareSymbol(b.container.symbol.exports, b.container.symbol, node, symbolFlags, symbolExcludes)
			...
			local := b.declareSymbol(b.container.locals /*parent*/, nil, node, exportKind, symbolExcludes)
...
func (b *Binder) bindContainer(node Mutable[HasContainerFlags], containerFlags ContainerFlags) {
	...
	if containerFlags & ContainerFlagsIsControlFlowContainer {
		...
		if !isImmediatelyInvoked {
			b.currentFlow = createFlowNode(FlowFlagsStart /*node*/, nil /*antecedent*/, nil)
...
func (b *Binder) bindCondition(node Expression, trueTarget FlowLabel, falseTarget FlowLabel) {
	...
	if !node || !b.isLogicalAssignmentExpression(node) && !b.isLogicalExpression(node) && !(isOptionalChain(node) && isOutermostOptionalChain(node)) {
...
func (b *Binder) findActiveLabel(name __String) *ActiveLabel {
	for label := b.activeLabelList; label; label = label.next {
		if label.name == name {
			return label
		}
	}
...
func (b *Binder) bindTryStatement(node TryStatement) {
	...
	if node.finallyBlock {
		...
		if b.currentFlow.flags & FlowFlagsUnreachable {
			// If the end of the finally block is unreachable, the end of the entire try statement is unreachable.
			b.currentFlow = b.unreachableFlow
		} else {
			// If we have an IIFE return target and return statements in the try or catch blocks, add a control
			// flow that goes back through the finally block and back through only the return statements.
			if b.currentReturnTarget && returnLabel.antecedent {
			...
			if b.currentExceptionTarget && exceptionLabel.antecedent {
...
func (b *Binder) bindCaseBlock(node CaseBlock) {
	...
	for i := 0; i < clauses.length; i++ {
		...
		b.addAntecedent(preCaseLabel, __COND__(isNarrowingSwitch, b.createFlowSwitchClause(b.preSwitchCaseFlow, node.parent, clauseStart, i+1), b.preSwitchCaseFlow))
		b.addAntecedent(preCaseLabel, fallthroughFlow)
		b.currentFlow = b.finishFlowLabel(preCaseLabel)
		clause := clauses[i]
		b.bind(clause)
		fallthroughFlow = b.currentFlow
		if !(b.currentFlow.flags & FlowFlagsUnreachable) && i != clauses.length-1 && b.options.noFallthroughCasesInSwitch {
...
func (b *Binder) declareSourceFileMember(node Declaration, symbolFlags SymbolFlags, symbolExcludes SymbolFlags) Symbol {
	if isExternalModule(b.file) {
		...
		return b.declareSymbol(b.file.locals /*parent*/, nil, node, symbolFlags, symbolExcludes)
	}
}
...
func (b *Binder) bindModuleDeclaration(node ModuleDeclaration) {
	...
	if isAmbientModule(node) {
		...
		if isModuleAugmentationExternal(node) {
			...
			b.file.patternAmbientModules = append(b.file.patternAmbientModules, __COND__(pattern && !isString(pattern), map[any]any{ /* TODO(TS-TO-GO): was object literal */
				"pattern": pattern,
				"symbol":  symbol,
			}, nil))
		}
...
func (b *Binder) declareModuleSymbol(node ModuleDeclaration) ModuleInstanceState {
	...
	b.declareSymbolAndAddToSymbolTable(node, __COND__(instantiated, SymbolFlagsValueModule, SymbolFlagsNamespaceModule), __COND__(instantiated, SymbolFlagsValueModuleExcludes, SymbolFlagsNamespaceModuleExcludes))
	return state
}
...
func (b *Binder) bindBlockScopedDeclaration(node Declaration, symbolFlags SymbolFlags, symbolExcludes SymbolFlags) {
	...
	default:
		...
		if !b.blockScopeContainer.locals {
			b.blockScopeContainer.locals = createSymbolTable()
			b.addToContainerChain(b.blockScopeContainer)
		}
		b.declareSymbol(b.blockScopeContainer.locals /*parent*/, nil, node, symbolFlags, symbolExcludes)
	}
}
...
func (b *Binder) checkContextualIdentifier(node Identifier) {
	...
	if !b.file.parseDiagnostics.length && !(node.flags & NodeFlagsAmbient) && !(node.flags & NodeFlagsJSDoc) && !isIdentifierName(node) {
...
func (b *Binder) getStrictModeIdentifierMessage(node Node) DiagnosticMessage {
...
func (b *Binder) checkPrivateIdentifier(node PrivateIdentifier) {
	if node.escapedText == "#constructor" {
		// Report error only if there are no parse errors in file
		if !b.file.parseDiagnostics.length {
...
func (b *Binder) checkStrictModeCatchClause(node CatchClause) {
	...
	if b.inStrictMode && node.variableDeclaration {
...
func (b *Binder) getStrictModeEvalOrArgumentsMessage(node Node) DiagnosticMessage {
	// Provide specialized messages to help the user understand why we think they're in
	// strict mode.
	if getContainingClass(node) {
...
func (b *Binder) checkStrictModeFunctionName(node FunctionLikeDeclaration) {
	if b.inStrictMode && !(node.flags & NodeFlagsAmbient) {
		// It is a SyntaxError if the identifier eval or arguments appears within a FormalParameterList of a strict mode FunctionDeclaration or FunctionExpression (13.1))
		b.checkStrictModeEvalOrArguments(node, node.name)
	}
}

func (b *Binder) getStrictModeBlockScopeFunctionDeclarationMessage(node Node) DiagnosticMessage {
	// Provide specialized messages to help the user understand why we think they're in
	// strict mode.
	if getContainingClass(node) {
...
func (b *Binder) bindWorker(node Node) /* TODO(TS-TO-GO) inferred type number | void | Symbol */ any {
	...
	case SyntaxKindPropertyAccessExpression,
		...
		if isInJSFile(expr) && b.file.commonJsModuleIndicator && isModuleExportsAccessExpression(expr) && !lookupSymbolForName(b.blockScopeContainer, "module" /* as __String */) {
...
func (b *Binder) bindExportAssignment(node ExportAssignment) {
	if !b.container.symbol || !b.container.symbol.exports {
...
func (b *Binder) setCommonJsModuleIndicator(node Node) bool {
	...
	if !b.file.commonJsModuleIndicator {
...
func (b *Binder) bindPrototypeAssignment(node BindableStaticPropertyAssignmentExpression) {
	...
	b.bindPropertyAssignment(node.left.expression, node.left /*isPrototypeProperty*/, false /*containerIsClass*/, true)
}
...
func (b *Binder) bindObjectDefinePrototypeProperty(node BindableObjectDefinePropertyCall) {
	...
	b.bindPotentiallyNewExpandoMemberToNamespace(node, namespaceSymbol /*isPrototypeProperty*/, true)
}
...
func (b *Binder) bindPrototypePropertyAssignment(lhs BindableStaticAccessExpression, parent Node) {
	...
	b.bindPropertyAssignment(constructorFunction, lhs /*isPrototypeProperty*/, true /*containerIsClass*/, true)
}
...
func (b *Binder) bindObjectDefinePropertyAssignment(node BindableObjectDefinePropertyCall) {
	...
	namespaceSymbol = b.bindPotentiallyMissingNamespaces(namespaceSymbol, node.arguments[0], isToplevel /*isPrototypeProperty*/, false /*containerIsClass*/, false)
	b.bindPotentiallyNewExpandoMemberToNamespace(node, namespaceSymbol /*isPrototypeProperty*/, false)
}
...
func (b *Binder) bindSpecialPropertyAssignment(node BindablePropertyAssignmentExpression) {
	...
	if isIdentifier(rootExpr) && lookupSymbolForName(b.container, rootExpr.escapedText). /* ? */ flags&SymbolFlagsAlias {
		return
	}
	// Fix up parent pointers since we're going to use these nodes before we bind into them
	setParent(node.left, node)
...
func (b *Binder) bindCallExpression(node CallExpression) {
	...
	if !b.file.commonJsModuleIndicator && isRequireCall(node /*requireStringLiteralLikeArgument*/, false) {
...
func (b *Binder) bindParameter(node /* TODO(TS-TO-GO) TypeNode UnionType: ParameterDeclaration | JSDocParameterTag */ any) {
	...
	if isParameterPropertyDeclaration(node, node.parent) {
		...
		b.declareSymbol(classDeclaration.symbol.members, classDeclaration.symbol, node, SymbolFlagsProperty|(__COND__(node.questionToken, SymbolFlagsOptional, SymbolFlagsNone)), SymbolFlagsPropertyExcludes)
	}
}
	...
	if b.currentFlow == b.unreachableFlow {
		...
		if reportError {
			...
			if !b.options.allowUnreachableCode {
				...
				isError := unreachableCodeIsError(b.options) && !(node.flags & NodeFlagsAmbient) && (!isVariableStatement(node) || !!(getCombinedNodeFlags(node.declarationList) & NodeFlagsBlockScoped) || node.declarationList.declarations.some(func(d VariableDeclaration) bool {
