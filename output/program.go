...
type CompilerHostLikeWithCache struct {
	...
	getSourceFileWithCache  *func(fileName string, languageVersionOrOptions Union[ScriptTarget, CreateSourceFileOptions], onError func(message string), shouldCreateNewSourceFile bool) *SourceFile
...
func changeCompilerHostLikeToUseCache(host CompilerHostLikeForCache, toPath func(fileName string) Path, getSourceFile /* TODO(TS-TO-GO) TypeNode IndexedAccessType: CompilerHost["getSourceFile"] */ any) CompilerHostLikeWithCache {
	...
	readFileCache := NewMap[Path, Union[string /* TODO(TS-TO-GO) TypeNode LiteralType: false */, any]]()
	...
	if getSourceFile != nil {
		getSourceFileWithCache = func(fileName string, languageVersionOrOptions /* TODO(TS-TO-GO) inferred type ScriptTarget | CreateSourceFileOptions */ any, onError * /* TODO(TS-TO-GO) inferred type ((message: string) => void) */ any, shouldCreateNewSourceFile *bool) *SourceFile {
			...
			if sourceFile != nil && (isDeclarationFileName(fileName) || fileExtensionIs(fileName, ExtensionJson)) {
	...
	// directoryExists
	if originalDirectoryExists != nil {
		...
		if originalCreateDirectory != nil {
...
func getPreEmitDiagnostics(program Union[Program, BuilderProgram], sourceFile SourceFile, cancellationToken CancellationToken) []Diagnostic {
...
func getModeForFileReference(ref Union[FileReference, string], containingFileMode ResolutionMode) ResolutionMode {
...
func isExclusivelyTypeOnlyImportOrExport(decl Union[ImportDeclaration, ExportDeclaration, JSDocImportTag]) bool {
...
func getModeForUsageLocationWorker(file Pick[SourceFile, Union[ /* TODO(TS-TO-GO) TypeNode LiteralType: "fileName" */ any /* TODO(TS-TO-GO) TypeNode LiteralType: "impliedNodeFormat" */, any /* TODO(TS-TO-GO) TypeNode LiteralType: "packageJsonScope" */, any]], usage StringLiteralLike, compilerOptions CompilerOptions) ResolutionMode {
	...
	if compilerOptions != nil && importSyntaxAffectsModuleResolution(compilerOptions) {
...
func forEachProjectReference(projectReferences *[]ProjectReference, resolvedProjectReferences *[]*ResolvedProjectReference, cbResolvedRef func(resolvedRef *ResolvedProjectReference, parent *ResolvedProjectReference, index number) *T, cbRef func(projectReferences *[]ProjectReference, parent *ResolvedProjectReference) *T) *T {
	...
	worker := func(projectReferences *[]ProjectReference, resolvedProjectReferences *[]*ResolvedProjectReference, parent *ResolvedProjectReference) *T {
		...
		return forEach(resolvedProjectReferences, func(resolvedRef *ResolvedProjectReference, index number) *T {
			if resolvedRef != nil && seenResolvedRefs. /* ? */ has(resolvedRef.sourceFile.path) {
			...
			if resolvedRef != nil && !skipChildren. /* ? */ has(resolvedRef) {
...
func isReferenceFileLocation(location Union[ReferenceFileLocation, SyntheticReferenceFileLocation]) bool {
	return (location.(ReferenceFileLocation)).pos != nil
}

/** @internal */

func getReferencedFileLocation(program Program, ref ReferencedFile) Union[ReferenceFileLocation, SyntheticReferenceFileLocation] {
...
func isProgramUptoDate(program *Program, rootFileNames []string, newOptions CompilerOptions, getSourceVersion func(path Path, fileName string) *string, fileExists func(fileName string) bool, hasInvalidatedResolutions HasInvalidatedResolutions, hasInvalidatedLibResolutions HasInvalidatedLibResolutions, hasChangedAutomaticTypeDirectiveNames *HasChangedAutomaticTypeDirectiveNames, getParsedCommandLine func(fileName string) *ParsedCommandLine, projectReferences *[]ProjectReference) bool {
	...
	if program.resolvedLibReferences != nil && forEachEntry(program.resolvedLibReferences, func(_value /* TODO(TS-TO-GO) inferred type LibResolution<ResolvedModuleWithFailedLookupLocations> */ any, libFileName string) bool {
	...
	if currentOptions.configFile != nil && newOptions.configFile != nil {
...
func createProgram(rootNamesOrOptions Union[[]string, CreateProgramOptions], _options CompilerOptions, _host CompilerHost, _oldProgram Program, _configFileParsingDiagnostics []Diagnostic) Program {
	...
	var reasonToRelatedInfo *Map[FileIncludeReason, Union[DiagnosticWithLocation /* TODO(TS-TO-GO) TypeNode LiteralType: false */, any]]
	...
	if host.getDefaultLibLocation != nil {
	...
	if host.resolveModuleNameLiterals != nil {
	...
	} else if host.resolveModuleNames != nil {
	...
	var projectReferenceRedirects *Map[Path, Union[ResolvedProjectReference /* TODO(TS-TO-GO) TypeNode LiteralType: false */, any]]
	...
	if structureIsReused != StructureIsReusedCompletely {
		...
		if rootNames.length != 0 && !skipDefaultLib {
	...
	if oldProgram != nil && host.onReleaseOldSourceFile != nil {
	...
	if oldProgram != nil && host.onReleaseParsedCommandLine != nil {
	...
	updateAndGetProgramDiagnostics := func() DiagnosticCollection {
		if lazyProgramDiagnosticExplainingFile != nil {
	...
	getPackagesMap := func() Map[string, bool] {
		...
		forEachResolvedModule(func(TODO_IDENTIFIER ResolvedModuleWithFailedLookupLocations) {
			if resolvedModule. /* ? */ packageId != nil {
	...
	addResolutionDiagnostics := func(resolution Union[ResolvedModuleWithFailedLookupLocations, ResolvedTypeReferenceDirectiveWithFailedLookupLocations]) {
	...
	resolveTypeReferenceDirectiveNamesWorker := func(typeDirectiveNames []T, containingFile Union[string, SourceFile], reusedNames *[]T) []ResolvedTypeReferenceDirectiveWithFailedLookupLocations {
	...
	getRedirectReferenceForResolution := func(file SourceFile) *ResolvedProjectReference {
		...
		if redirect != nil || !isDeclarationFileName(file.originalFileName) {
			return redirect
		}
	...
	resolveTypeReferenceDirectiveNamesReusingOldState := func(typeDirectiveNames []T, containingFile Union[string, SourceFile]) []ResolvedTypeReferenceDirectiveWithFailedLookupLocations {
		var containingSourceFile *SourceFile
		if !isString(containingFile) {
			containingSourceFile = containingFile
		} else {
	...
	canReuseProjectReferences := func() bool {
		return !forEachProjectReference(oldProgram.getProjectReferences(), oldProgram.getResolvedProjectReferences(), func(oldResolvedRef *ResolvedProjectReference, parent *ResolvedProjectReference, index number) bool {
			newRef := (ifElse(parent != nil, parent.commandLine.projectReferences, projectReferences))[index]
	...
	tryReuseStructureFromOldProgram := func() StructureIsReused {
		...
		if oldProgram.resolvedLibReferences != nil && forEachEntry(oldProgram.resolvedLibReferences, func(resolution /* TODO(TS-TO-GO) inferred type LibResolution<ResolvedModuleWithFailedLookupLocations> */ any, libFileName string) bool {
			return pathForLibFileWorker(libFileName).actual != resolution.actual
		}) {
			return StructureIsReusedSafeModules
		}

		if host.hasChangedAutomaticTypeDirectiveNames != nil {
	...
	getBindAndCheckDiagnosticsForFileNoCache := func(sourceFile SourceFile, cancellationToken CancellationToken, nodesToCheck []*Node) []Diagnostic {
		return runWithCancellationToken(func() []Diagnostic {
			...
			return getMergedBindAndCheckDiagnostics(sourceFile, !isPlainJs, nodesToCheck != nil, bindDiagnostics, checkDiagnostics, ifElse(isCheckJs, sourceFile.jsDocDiagnostics, nil))
		})
	}
	...
	collectExternalModuleReferences := func(file SourceFile) {
		...
		var moduleAugmentations *[]Union[StringLiteral, Identifier]
	...
	getSourceFileFromReferenceWorker := func(fileName string, getSourceFile func(fileName string) *SourceFile, fail func(diagnostic DiagnosticMessage, argument []string), reason FileIncludeReason) *SourceFile {
		if hasExtension(fileName) {
			...
			if fail != nil && options.allowNonTsExtensions {
				fail(Diagnostics.File_0_not_found, fileName)
				return nil
			}
	...
	findSourceFileWorker := func(fileName string, isDefaultLib bool, ignoreNoDefaultLib bool, reason FileIncludeReason, packageId *PackageId) *SourceFile {
		...
		if useSourceOfProjectReferenceRedirect {
			...
			// Note:: Currently we try the real path only if the
			// file is from node_modules to avoid having to run real path on all file paths
			if !source && host.realpath != nil && options.preserveSymlinks && isDeclarationFileName(fileName) && fileName.includes(nodeModulesPathPart) {
	...
	addFileIncludeReason := func(file *SourceFile, reason FileIncludeReason, checkExisting bool) bool {
		if file != nil && (!checkExisting || !isReferencedFile(reason) || !filesWithReferencesProcessed. /* ? */ has(reason.file)) {
	...
	isSourceOfProjectReferenceRedirect := func(fileName string) bool {
		return useSourceOfProjectReferenceRedirect && getResolvedProjectReferenceToRedirect(fileName) != nil
	}
	...
	processTypeReferenceDirectiveWorker := func(typeReferenceDirective string, mode ResolutionMode, resolution ResolvedTypeReferenceDirectiveWithFailedLookupLocations, reason FileIncludeReason) {
		...
		if resolvedTypeReferenceDirective != nil {
	...
	pathForLibFileWorker := func(libFileName string) LibResolution {
		...
		if structureIsReused != StructureIsReusedNot && oldProgram != nil && !hasInvalidatedLibResolutions(libFileName) {
	...
	processImportedModules := func(file SourceFile) {
		...
		if file.imports.length != 0 || file.moduleAugmentations.length != 0 {
	...
	verifyCompilerOptions := func() {
		...
		if options.lib != nil && options.noLib {
	...
	createDiagnosticExplainingFile := func(file *SourceFile, fileProcessingReason *FileIncludeReason, diagnostic DiagnosticMessage, args DiagnosticArguments) Diagnostic {
		...
		var chain *DiagnosticMessageChain
		if cachedChain != nil {
			if cachedChain.fileIncludeReasonDetails != nil {
		...
		if locationReason != nil && seenReasons. /* ? */ size == 1 {
			seenReasons = nil
		}

		if seenReasons != nil && cachedChain != nil {
			...
			} else if cachedChain.fileIncludeReasonDetails != nil {
		...
		if location != nil && isReferenceFileLocation(location) {
	...
	fileIncludeReasonToRelatedInformation := func(reason FileIncludeReason) *DiagnosticWithLocation {
		...
		case FileIncludeKindSourceFromProjectReference,
			...
			if referencesSyntax != nil && referencesSyntax.elements.length > index {
	...
	createCompilerOptionsDiagnostic := func(message Union[DiagnosticMessage, DiagnosticMessageChain], args DiagnosticArguments) {
	...
	createOptionDiagnosticInObjectLiteralSyntax := func(objectLiteral ObjectLiteralExpression, onKey bool, key1 string, key2 *string, message Union[DiagnosticMessage, DiagnosticMessageChain], args DiagnosticArguments) bool {
	...
	isEmittedFile := func(file string) bool {
		...
		if getSourceFileByPath(filePath) != nil {
			return false
		}
		...
		if fileExtensionIsOneOf(filePath, supportedJSExtensionsFlat) || isDeclarationFileName(filePath) {
			...
			return getSourceFileByPath((filePathWithoutExtension+ExtensionTs).(Path)) != nil || getSourceFileByPath((filePathWithoutExtension+ExtensionTsx).(Path)) != nil
		}
		return false
	}
...
func shouldTransformImportCallWorker(sourceFile Pick[SourceFile, Union[ /* TODO(TS-TO-GO) TypeNode LiteralType: "fileName" */ any /* TODO(TS-TO-GO) TypeNode LiteralType: "impliedNodeFormat" */, any /* TODO(TS-TO-GO) TypeNode LiteralType: "packageJsonScope" */, any]], options CompilerOptions) bool {
	moduleKind := getEmitModuleKind(options)
...
func getImpliedNodeFormatForEmitWorker(sourceFile Pick[SourceFile, Union[ /* TODO(TS-TO-GO) TypeNode LiteralType: "fileName" */ any /* TODO(TS-TO-GO) TypeNode LiteralType: "impliedNodeFormat" */, any /* TODO(TS-TO-GO) TypeNode LiteralType: "packageJsonScope" */, any]], options CompilerOptions) ResolutionMode {
	moduleKind := getEmitModuleKind(options)
...
func getDefaultResolutionModeForFileWorker(sourceFile Pick[SourceFile, Union[ /* TODO(TS-TO-GO) TypeNode LiteralType: "fileName" */ any /* TODO(TS-TO-GO) TypeNode LiteralType: "impliedNodeFormat" */, any /* TODO(TS-TO-GO) TypeNode LiteralType: "packageJsonScope" */, any]], options CompilerOptions) ResolutionMode {
...
func handleNoEmitOptions(program Union[Program, T], sourceFile *SourceFile, writeFile *WriteFileCallback, cancellationToken CancellationToken) *EmitResult {
